...
class HumConverter():
    ...
    def fillRootFile(self, chunksize=None):
        ...
        if self.checkTreeExists() is False:
            ...
            outputTree = ROOT.TTree(self.treeNames[0], "Humidity measured by DHT22 sensor")
            valuesToFill = {}
            ...
            outputFile.Close()
        print(f"Start filling: '{self.outputRootFileName}' from file: '{self.humFilename}'")
        outputFile = ROOT.TFile(f"{self.outputRootFileName}", "UPDATE")
        ...
        self.df = pd.read_csv(self.humFilename, sep=",", header=0, names=self.header, on_bad_lines="warn")
        print(f"{len(self.df)} entries in total.")
        with tqdm(total=len(self.df)) as pbar:
            for index, row in self.df.iterrows():
                for element in self.df.columns:
                    try:
                        valuesToFill[f"{element}"][0] = float(row[element])
                        outputTree.Fill()
                        pbar.update(1)
                    except:
                        pbar.update(1)
